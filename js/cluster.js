...
Cluster.BuildClusterCapsule = function( unAppID, unPackageID, strStatus, strFeature, nDepth, rgItemInfo )
{
	...
	{
		$CapCtn.append( $J('<img/>', {'class': 'cluster_capsule_image', src: 'https://static-cdn.viga.me/images/maincap_placeholder_616x353.gif', 'data-image-url': rgItemData.main_capsule } ) );
	}
	else
	{
		...
		if ( strImageURL )
		{
			$CapCtn.append( $J('<div/>', {'class': 'cluster_maincap_fill ' + (rgItemData.package_header ? 'package' : '') } )
					.append(
					$J('<img/>', {'class': 'cluster_maincap_fill_placeholder', src: 'https://static-cdn.viga.me/images/maincap_placeholder_616x353.gif' } ),
					$J('<img/>', {'class': 'cluster_capsule_image cluster_maincap_fill_bg', src: 'https://static-cdn.viga.me/images/blank.gif', 'data-image-url': strImageURL } ),
					$J('<img/>', {'class': 'cluster_maincap_fill_header', src: 'https://static-cdn.viga.me/images/blank.gif', 'data-image-url': strImageURL } )
				)
			);
		}
		else
	...
	if ( rgItemInfo.recommended || rgItemInfo.recommended_by_curator )
	{
		...
		$elWhyButton.click ( function ( event )
		{
			...
			$J.ajax ( {
				url: "https:\/\/viga.me\/explore\/recommendedreason\/",
				data: {
					appid: unAppID
				},
				dataType: 'json',
			...
			} ).done ( function ( data )
			{
				...
				if ( data.tags )
				{
					...
					{
						strTooltipContent += '<a href="https://viga.me/tag/en/'+data.tags[ i ].name+'/">' + data.tags[ i ].name + '</a>';
					}
				}
