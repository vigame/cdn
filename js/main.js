...
{
	var win = window.open( 'https://viga.me/gotflash','gotflash','width=536,height=546,resize=yes,scrollbars=yes');
	win.focus();
}
...
{	
	...
	if ( elem.curPos > elem.maxLoaded )
	{
		...
		new Ajax.Updater( 
				targetid, 
				'https://viga.me/search/smallcapscroll', 
		...
		if ( !$HoverData.length && !oElemState.bAjaxRequestMade )
		{
			...
			window.setTimeout( function() {
				if ( oElemState.bWantsHover && !oElemState.bAjaxRequestMade ) {
					...
					$J.get( 'https://viga.me/' + strUrlTarget, rgAjaxParams ).done( function( html )
					{
						var $Content = $J(html);
						$Content.hide();
...
{
	var url = 'https://viga.me/api/addtowishlist';
	if ( navref )
	{
...
{
	$J.post( 'https://viga.me/recommended/ignorerecommendation/', {
...
function RecommendGame( appid, steamworksappid, comment, rated_up, is_public, language, received_compensation, divBtn, onSuccessFunc, divToShowError, navref )
{
	...
	var url = 'https://viga.me/friends/recommendgame';
	if ( navref )
		MakeNavCookie( navref, url );
...
function HideRecommendation( type, itemid, divBtn, elemContainer )
{
	...
	new Ajax.Request( 'https://viga.me/recommended/ignorerecommendation/', {
		method: 'post',
...
function SearchTimeout( $Term, value, $SuggestionsCtn, $Suggestions )
{
	if ( value )
	{
		...
		$J.get( 'https://viga.me/search/suggest', parameters).done( function( html ) {
...
function EnsureStoreMenuTagsLoaded( strId )
{
	...
	if ( !$Element.data('tags-loaded') )
	{
		...
		var url = 'https://viga.me/tagdata/recommendedtags';
		...
		$J.get( url, {ll: 'english'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'https://viga.me/tag/en/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.post( 'https://viga.me/checkout/addfreelicense/' + subid, { ajax: true, sessionid: g_sessionID }).done( function() {
		ShowAlertDialog(
			strDisplayName,
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://viga.me/account/setlanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
	.done( function() {
		if ( bStayOnPage )
			Modal.Dismiss();
		else
		{
						if( g_AccountID )
				window.location = 'https://viga.me/account/languagepreferences/';
...
function Logout()
{
	var $Form = $J('<form/>', {'action': 'https://viga.me/logout/', 'method': 'POST' } );
...
function RecordAJAXPageView( url )
{
	...
	{
		var rgURLs = [ 'https://viga.me/', 'https://viga.me/' ];
