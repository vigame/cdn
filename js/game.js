...
function CreateWidget( nAppId )
{
	...
	var strWidgetURL;
	if( nSubId )
		strWidgetURL = 'https://viga.me/widget/' + nAppId + '/' + nSubId + '/';
	else
		strWidgetURL = 'https://viga.me/widget/' + nAppId + '/';
...
function InitQueueControls( store_appid, steamworks_appid, next_in_queue_appid, snr )
{
	...
	$FollowBtn.click( function() {
		$J.post( 'https://viga.me/explore/followgame/', {
			sessionid: g_sessionID,
	...
	$NextInQueueBtn.click( function() {
		if ( bQueueIsValid )
			...
			window.location = 'https://viga.me/explore/startnew';
	});
		...
		CDiscoveryQueue.ShowCustomizeDialog( function( data ) {
				...
				( next_in_queue_appid && data.queue.indexOf( next_in_queue_appid ) == -1 ) ) )
			{
				ShowConfirmDialog( 'Customize Your Discovery Queue', 'We\'ve built you a new Discovery Queue by applying your customizations.  What would you like to do?',
					...
					window.location = 'https://viga.me/explore/startnew/' + g_eDiscoveryQueueType + '/';
				});
				bQueueIsValid = false;
			}
			else
...
function RequestCurrentUserRecommendationVotes( recommendationIDs )
{
	...
	$J.post( 'https://viga.me/userreviews/ajaxgetvotes/', {
...
{
	UserReview_Rate( id, true, 'https://viga.me/',
		function( rgResults ) {
...
{
	UserReview_Rate( id, false, 'https://viga.me/',
		function( rgResults ) {
			if ( rgResults.nSaleTaskCompleted ) { NewStickerPackModal( 'Mark a Review as Helpful ... or not');}
...
function UserReviewVoteTag( id, tagID, elemID )
{
	...
	UserReview_VoteTag( id, tagID, bRateUp, 'https://viga.me/',
		function( rgResults ) {
...
function UserReviewSetQuality( id )
{
	...
	UserReview_Moderate_SetQuality( id, quality, 'https://viga.me/',
		function( rgResults ) {
		}
	);
}
...
function LoadMoreReviews( appid, startOffset, dayRange, startDate, endDate, context )
{
	...
	$J.get( 'https://viga.me/appreviews/' + appid,{
...
function BuildReviewHistogram()
{
	...
	$J.get( 'https://viga.me/appreviewhistogram/' + appid, { l: 'english' }
	).done( function( data ) {
...
function ShowReportDialog( nAppId )
{
	...
	dialog.done( function() {
		...
		{
			$J.post(
				'https://viga.me/appreport/' + nAppId + '/report/',
				{
