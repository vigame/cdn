...
GHomepage = {
	...
	OnHomeActivate: function()
	{
		...
		try {
			...
			{
				$J.ajax( {
					url: "https:\/\/viga.me\/default\/home_additional\/",
					data: {
	...
	OnDynamicStoreReady: function()
	{
		...
		if( bHaveUser )
		{
			$J('#content_more').autoloader({template_url: 'https://viga.me/explore/render/', recommendations_url: 'https://viga.me/explore/recommended/', additional_url: 'https://viga.me/explore/more'});
		}
		else
		{
			$J('#content_more').hide();
		...
		$J('.tab_item').each(function(i, j){
			...
			if( unAppId && GStoreItemData.rgAppData[ unAppId ] )
			{
				...
				if( rgData.tags )
				{
					...
					{
						var url = GStoreItemData.AddNavEventParamsToURL( 'https://viga.me/tag/en/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgData.tags[i] ) ), 'tab-preview' );
	...
	RenderRecommendedTags: function()
	{
		...
		for ( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length; i++ )
		{
			...
			{
				var url = GStoreItemData.AddNavEventParamsToURL( 'https://viga.me/tag/en/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgTag.name ) ), 'gutter' );
...
CHomeSettings.prototype.OnCheckboxToggle = function( strSettingName, $Checkbox )
{
	...
	var _this = this;
	$J.post( 'https://viga.me/dynamicstore/updatehomeviewsettings', {
...
CUsabilityTracker.prototype.PostStats = function()
{
	...
	var strURL = "http:\/\/viga.me\/default\/usabilitytracking\/";
