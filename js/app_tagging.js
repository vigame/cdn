...
function InitAppTagModal( appid, rgAppTags, rgUserTags, strTagLinkSNR, strYourTagSNR, bShowBanOption )
{
		...
		if ( bPopular )
		{
			...
			if ( $YourTags.length )
			{
				...
				if ( bShowBanOption )
				{
					...
					$Ban.data('store-tooltip', 'Click here to remove this tag from this product.  This option is limited to developers and Viga.Me staff.' );
	...
	var fnBuildTagDisplay = function( Modal )
	{
		...
		{
			$YourTags.html( '<div class="yourtags_throbber"><img src="https://static-cdn.viga.me/images/throbber.gif"></div>' );
			if ( !bYourPopularTagsRequested )
			{
				bYourPopularTagsRequested = true;

				var url = 'https://viga.me/tagdata/myfrequenttags';
				...
				$J.get( 'https://viga.me/tagdata/populartags/english').done( function ( data ) {
	...
	var fnApplyTag = function( tag, withdraw, bPopularClick, eReportType, unTagID )
	{
		...
		$J.post( 'https://viga.me/tagdata/tagapp', rgParams ).done( function( data ) {
...
function InitTagBrowsePage( strTagLanguage, rgDefaultGetParams )
{
	...
	var fnDisplayTag = function( unTagID, strTagName ) {
		if ( !rgTagResults[unTagID] )
		{
			...
			$GamesElement.append( $J('<div/>', {'class': 'browse_tag_loading' }).html( '<img src="https://static-cdn.viga.me/images/throbber.gif">' ) );
			...
			$J.get( 'https://viga.me/tagdata/gettaggames/' + strTagLanguage + '/' + unTagID + '/', rgParams ).done( function ( html ) {
				$Element.html( html );
	...
	if ( g_AccountID != 0 )
	{
		...
		$J.get( 'https://viga.me/tagdata/recommendedtags', {games: 10, count: 25, taglang: strTagLanguage }).done( function( data ) {
