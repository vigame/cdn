...
GDynamicStore = {
	...
	Init: function( accountid, bForceRefresh, strOS )
	{
		...
		if ( accountid )
		{
			...
			var url = 'https://viga.me/dynamicstore/userdata/?id=' + accountid;
	...
	CapsuleSettingsMenu: function( elSource )
	{
		...
		if( strAppIDs )
		{
			...
			var $elPreferences = $J ( '<a/>' ).attr('href', 'https://viga.me/preferences' ).text ( "Preferences" ).addClass( 'option' );
	...
	ModifyWishlist: function( appid, bRemove, fnOnSuccess, fnOnFail )
	{
		...
		if( bRemove )
			url = 'https://viga.me/api/removefromwishlist';
...
GStoreItemData = {
	...
	GetAppURL: function( unAppID, strFeatureContext, nDepth )
	{
		if ( typeof GStoreItemData.rgAppData[ unAppID ] == 'object' && 'url_name' in GStoreItemData.rgAppData[ unAppID ] ) {
			return GStoreItemData.AddNavEventParamsToURL( 'https://viga.me/app/' + unAppID + '/' + GStoreItemData.rgAppData[ unAppID ].url_name + '/', strFeatureContext, nDepth )
		}

		return GStoreItemData.AddNavEventParamsToURL( 'https://viga.me/app/' + unAppID + '/', strFeatureContext, nDepth )
	},

	GetPackageURL: function( unPackageID, strFeatureContext, nDepth )
	{
		return GStoreItemData.AddNavEventParamsToURL( 'https://viga.me/sub/' + unPackageID + '/', strFeatureContext, nDepth )
	},

	GetBundleURL: function( unBundleID, strFeatureContext, nDepth )
	{
		return GStoreItemData.AddNavEventParamsToURL( 'https://viga.me/bundle/' + unBundleID + '/', strFeatureContext, nDepth )
	},
...
{
	$J.get(
		'https://viga.me/explore/howitworks/',
		{
			l : 'english'
		},
		function( data )
